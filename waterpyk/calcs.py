...
def combine_bands(df, bands_to_combine, band_name_final):
    ...
    Args:
        bands_to_combine (:obj:`list` of :obj:`str`): list of strings of band names (example: ['Es', 'Ec']) to combine into a single band
        band_name_final (:obj:`str`): the final band name to be stored in the 'band' column of the final dataframe, where the 'value' column will contain the added values from bands_to_combine
    
    Returns:
       :obj:`df`: input long-form dataframe with appended section which contains date, asset_name, value, band, and value_raw (set to np.nan) columns with data from combined
    """
...
def make_wide_df(df_long, **kwargs):
    ...
    Kwargs:
        ...
        ppt_asset (:obj:`str`): asset name for precipitation column (default 'prism').
        ppt_band (:obj:`str`): band name for precipitation colum (default 'ppt')
...
def merge(df_wide, df_merge, merge_with, column_names = None):
    ...
    Args:
        ...
        merge_with (:obj:`str`): choice of 'streamflow' or 'deficit' to merge with df_wide.
        column_names (:obj:`list` of :obj:`str`): Names of the column in df_merge that you wish to merge (default for streamflow: ['Q_mm'], default for deficit: ['D', and 'D_wy']). 'date' is automatically included.
